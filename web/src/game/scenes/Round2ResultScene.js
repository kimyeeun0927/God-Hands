...
export class Round2ResultScene {
  hideScoreHUD = true;
  ...
  render(handState) {
    ...
    ctx.fillText('ROUND 2', W / 2, H * 0.3);
    ctx.restore();
    ...
    if (isFist && !cooling) {
      ...
      ctx.textAlign    = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('✊ 감지됨 — 손을 펴서 Round 3 시작', W / 2, H * 0.83);
    } else {
      const pulse = 0.5 + 0.5 * Math.sin(this._elapsed * 0.003);
      ctx.globalAlpha  = 0.5 + 0.5 * pulse;
      ctx.font         = `${Math.floor(H * 0.02)}px 'Mulmaru', sans-serif`;
      ctx.fillStyle    = 'rgba(245,230,195,0.6)';
      ctx.textAlign    = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('주먹을 쥐었다 펴서 Round 3 시작 ▶', W / 2, H * 0.83);
    }
    ctx.restore();
  }
